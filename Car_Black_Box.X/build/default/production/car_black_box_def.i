...
char login(unsigned char reset_flag, unsigned char key)
{
    ...
    if (i == 4)
    {
        ...
        if (!strncmp(saved_passwd, new_passwd, 4))
        {
            return 0x01;
        }
        else
        {
            ...
            if (attempt_rem == 0)
            {
                ...
                clcd_print ("for 900 sec..", (0xC0 + 0));
                clcd_write(0x0C, 0);
                _delay((unsigned long)((100)*(20000000/4000000.0)));
                sec = 900;
                while(sec)
                {
                    clcd_putch(sec / 100 + '0', (0xC0 + 13));
                    clcd_putch(sec % 100 / 10 + '0', (0xC0 + 14));
                    clcd_putch(sec % 100 % 10 + '0', (0xC0 + 15));
                }
                attempt_rem = 3;
            }
            else
            {
                ...
                clcd_print("attempt remain", (0xC0 + 2));
...
void print_log(char log_pos)
{
    ...
    clcd_putch(saved_event[0], (0xC0 + 11));
    clcd_putch(saved_event[1], (0xC0 + 12));

    clcd_putch(saved_speed[0], (0xC0 + 14));
    clcd_putch(saved_speed[1], (0xC0 + 15));
...
char view_log(unsigned char reset_flag, unsigned char key)
{
    ...
    else
    {
        ...
        clcd_print("  ARE AVAILABLE ", (0xC0 + 0));
        _delay((unsigned long)((3000)*(20000000/4000.0)));
        return 0x22;
    }
    _delay((unsigned long)((81)*(20000000/4000.0)));
    return 0x11;
}
...
void download_log(void)
{
    ...
    if (log_count > 0)
    {
        ...
        clcd_print(" LOGS DISPLAYED ", (0x80 + 0));
        clcd_print("     IN UART    ", (0xC0 + 0));
    }
    else
    {
        clcd_print("     NO LOGS    ", (0x80 + 0));
...
void edit_time(unsigned char reset_flag, unsigned char key)
{
    ...
    if (!blink_flag)
    {
        clcd_putch(new_hr / 10 + '0', (0xC0 + 4));
        clcd_putch(new_hr % 10 + '0', (0xC0 + 5));
        clcd_putch(':', (0xC0 + 6));
        clcd_putch(new_min / 10 + '0', (0xC0 + 7));
        clcd_putch(new_min % 10 + '0', (0xC0 + 8));
        clcd_putch(':', (0xC0 + 9));
        clcd_putch(new_sec / 10 + '0', (0xC0 + 10));
        clcd_putch(new_sec % 10 + '0', (0xC0 + 11));
    }
    else
    {
        clcd_putch(0xFF, (0xC0 + j));
        clcd_putch(0xFF, (0xC0 + j + 1));
    }
}
...
char change_password(unsigned char reset_flag, unsigned char key)
{
    ...
    if (i == 4)
    {
        i++;
        clear_screen();
        clcd_print("RE-ENTER", (0x80 + 4));
        clcd_putch(' ', (0xC0 + 5));
    }
    else if (j == 4)
    {
        if (!strncmp(new_passwd_1, new_passwd_2, 4))
        {
            clear_screen();
            for (int i = 0 ; i < 4 ; i++)
            {
        ...
        {
            i = j = 0;
            clcd_print("PASSWORD DIDN'T ", (0x80 + 0));
            clcd_print("MATCH, TRY AGAIN", (0xC0 + 0));
             _delay((unsigned long)((3000)*(20000000/4000.0)));
            clear_screen();
            clcd_print(" ENTER PASSWORD ", (0x80 + 0));
            clcd_putch(' ', (0xC0 + 5));
