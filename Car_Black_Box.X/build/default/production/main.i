...
char pre_key, pre_control_flag = 0x00;
int press = 0;
extern char time[7];
...
void main(void) {
    ...
    char event[3] = "ON";
    unsigned char key, speed = 0, gr = 0;
    ...
    while (1) {
        ...
        if ((key != 0x3F) && (key != 0x1F))
        {
            if (pre_control_flag != 0x04)
            {
                press++;
                pre_key = key;
            }
        ...
        if (key == 0x3F)
        {
            ...
            {
                if (pre_key == 0x3E && strcmp(event, "C "))
                {
                    strcpy(event, "C ");
                    log_car_event(event, speed);
                    pre_key = press = 0;
                ...
                else if (pre_key == 0x37 || pre_key == 0x2F)
                {
                    clear_screen();
        ...
        if (press > 13)
        {
            if (pre_key == 0x37)
            {
                ...
                else if (control_flag == 0x08 || control_flag == 0x80)
                {
                    control_flag = 0x04;
                    reset_flag = 0x05;
                    clcd_write(0x0C, 0);
                ...
                else if (control_flag == 0x40)
                {
                    control_flag = 0x04;
                    reset_flag = 0x05;
                    write_ds1307(0x00, (new_sec / 10 << 4) | (new_sec % 10));
                    write_ds1307(0x01, (new_min / 10 << 4) | (new_min % 10));
                    write_ds1307(0x02, (new_hr / 10 << 4) | (new_hr % 10));
                }
                pre_key = press = 0;
            }
            ...
            {
                if (control_flag == 0x04 || control_flag == 0x08 || control_flag == 0x40 || control_flag == 0x80)
                {
                    clear_screen();
                    pre_control_flag = 0x04;
                    control_flag = 0x01;
